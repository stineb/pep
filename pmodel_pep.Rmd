...
df_zani <- df_zani %>% 
  ...
  left_join(df_orig, by = c("s_id", "species", "year"))
...
XXX ALL BELOW IS NOT YET UPDATED
...
df_comb <- df_pmodel %>% 
  unnest(out_pmodel) %>% 
  right_join(df, by = c("id_species_site", "year"))

df_comb %>% analyse_modobs2("cA_tot", "gpp", type = "heat")

df_comb %>% 
  ggplot(aes(x = cA_tot, y = gpp, color = off)) + 
  geom_point()
```
