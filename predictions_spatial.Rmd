...
```{r}
unique(df_sampled$lon_mid)
sort(unique(df_sampled$lat))
...
tmin <- tmin %>% mutate(doy=yday(date),year=year(date)) %>% rename(id_site=sitename) %>% mutate(id_site=as.integer(id_site))
tmin <- tmin %>% pivot_wider(names_from = doy, values_from = tmin) 
#tmin.df <- tmin %>% inner_join(df_sampled[,c(3,4,5,7)]) %>% distinct() %>% relocate(year,doy,id_site,lon,lat) %>% arrange(id_site,year) %>% dplyr::select(-date)
...
for(i in 1:length(all.latitudes)) {
  dl <- daylength(all.latitudes[i], 1:365)
  df_photo[[i]] <- dl 
}
...
#photo.df <- photo %>% full_join(df_sampled[,c(3,4,5,7)]) %>% distinct() %>% relocate(year) %>% relocate(id_site, .after = doy) %>% arrange(id_site,year) %>% relocate(lon, .after = id_site) 
...
for(i in 1:length(sps)) {
  ...
  for(site in sites){
    ...
    data = list("doy" = as.vector(pheno_sp.sub[index,]$on),
                ...
                "Ti" = NULL,
                "Tmini" = tmin,  #t(pheno_sp.sub[index,paste0(1:366,".x")])
                "Tmaxi" = NULL,
                "Li" = photo,
                "SPEI" = NULL,
                "VPDi" = NULL,
...
for(i in 1:2) { #length(species)
  ...
  for(site in 1:2) { #length(sites)
    ...
    ts <- paste0(sps[i],"_",sites[site])
    data.sub <- DataList[[i]][[site]]
    preds.sub <- preds.df #%>% filter(id_species_site==ts)
