...
<!-- Mine is correct! But I haven't check the implication of this for analyses. -->
...
LMM spatial
```{r}
fit_spatial_cA_tot = lm(off ~ cA_tot , data = df_spatial, na.action = "na.exclude")
summary(fit_spatial_cA_tot)
plot(allEffects(fit_spatial_cA_tot))
r.squaredGLMM(fit_spatial_cA_tot)

fit_spatial_cGSI = lm(off ~ cGSI , data = df_spatial, na.action = "na.exclude")
summary(fit_spatial_cGSI)
plot(allEffects(fit_spatial_cGSI))
r.squaredGLMM(fit_spatial_cGSI)
```

#### Drivers of spatial photosynthesis variations
...
```
Yes. They are definitely in the mountains. And mostly Larix. Where else are they?
```{r}
plot_map_simpl(lonmin = 0, lonmax = 30, latmin = 40, latmax = 60) +
