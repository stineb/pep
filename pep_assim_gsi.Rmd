...
```{r}
df_problem <- df_tmp %>%
  dplyr::filter(on.x != on.y) %>%
  dplyr::select(id_species_site, year, on_theirs = on.x, on_mine = on.y)
df_problem
```
...
```{r}
df_wide <- df_long %>% 
  pivot_wider(names_from = "pheno", values_from = day_correct)

df <- df %>% 
  rename(on_zani = on, off_zani = off) %>% 
  left_join(df_wide, by = c("id_species_site", "year"))

out <- df %>% analyse_modobs2("on", "on_zani")
out$gg
out <- df %>% analyse_modobs2("off", "off_zani")
out$gg
