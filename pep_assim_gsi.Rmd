...
Fit_IAV1 = lmer(off ~ cA_tot + (1|id_site/species) , data = df_anl, na.action = "na.exclude")
summary(Fit_IAV1)
r.squaredGLMM(Fit_IAV1)
plot(allEffects(Fit_IAV1))

Fit_IAV2 = lmer(off ~ cA_tot + (1|id_species_site) + (1|id_site), data = df_anl, na.action = "na.exclude")
summary(Fit_IAV2)
...
Fit_LT = lmer(off ~ scale(cA_tot) * scale(year) + (1|id_site/species), data = df_anl, na.action = "na.exclude")
summary(Fit_LT) 
#plot(allEffects(Fit_LT))
plot_model(Fit_LT, type = "eff", terms = c("cA_tot","year[2020,1960,1890]"),title = "",axis.title = c("cA_tot","off"),legend.title = "year") + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "bottom") 
...
plot_model(Fit_LT_anom, type = "eff", terms = c("on", "year[2020,1960,1890]"), title = "",axis.title = c("on","off"), legend.title = "year") + 
  theme_bw() + 
...
```{r}
Fit_bySite = lmer(off ~ cA_tot + (1|id_site) , data = df_anl, na.action = "na.exclude")
summary(Fit_bySite)
...
Fit_bySps = lmer(off ~ cA_tot + (1|species), data = df_spatial_sps, na.action = "na.exclude")
summary(Fit_bySps)
