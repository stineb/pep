...
```{r}
df_long <- data.table::fread("data/df_sub.csv") %>%  # "~/data/pep/processed/DataMeta_2_PhenologyObs_PEP725_CleanData.csv"
  as_tibble() %>%
...
## correct Zani: overwrite SOS and EOS with correct one
df_mine <- read_csv("data/df_anl_zani.csv")




df_correct <- data.table::fread("data/df_sub.csv") %>%  # "~/data/pep/processed/DataMeta_2_PhenologyObs_PEP725_CleanData.csv"
  as_tibble()

any(!(df_correct$s_id %>% unique() %in% df$id_site %>% unique()))
any(!(df$id_site %>% unique() %in% df_correct$s_id %>% unique()))

df_correct <- df_correct %>% 
  select(id_site = s_id, species, pheno, day, year) %>% 
  distinct(id_site, species, year, pheno, .keep_all = TRUE) %>% 
  pivot_wider(names_from = "pheno", values_from = "day") %>% 
  drop_na(on, off)

df_anl <- df %>% 
  dplyr::select(-on, -off) %>%
  left_join(df_mine, by = c("id_site", "species", "year"))

sum(!(df_correct$id_species_site %in% df$id_species_site))
sum(!(df$id_species_site %in%  df_correct$id_species_site ))
sum(is.na(df_anl$off))
sum(is.na(df_anl$on))
