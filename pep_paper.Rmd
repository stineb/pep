...
```{r}
Fit_IAV_on = lmer(off ~ scale(on) + (1|id_site) + (1|species) , data = df_anl_Zani, na.action = "na.exclude")
summary(Fit_IAV_on)
r.squaredGLMM(Fit_IAV_on)
plot(allEffects(Fit_IAV_on))
...
```{r}
Fit_IAV_cA_tot = lmer(off ~ scale(cA_tot) + (1|id_site) + (1|species) , data = df_anl_Zani, na.action = "na.exclude")
...
```{r}
Fit_LT_on = lmer(off ~ scale(on) + scale(year) + (1|id_site) + (1|species), data = df_anl_Zani, na.action = "na.exclude")
summary(Fit_LT_on)
...
```{r}
mod_on_year <- lmer(on ~ scale(year) + (1|id_site) + (1|species), data = df_anl_Zani, na.action = "na.exclude")
summary(mod_on_year)
...
```{r}
Fit_LT_cA_tot = lmer(off ~ scale(cA_tot) + scale(year) + (1|id_site) + (1|species), data = df_anl_Zani, na.action = "na.exclude")
...
mod_anet_year <- lmer(cA_tot ~ year + (1|id_site) + (1|species), data = df_anl_Zani, na.action = "na.exclude")

summary(mod_anet_year)
plot(allEffects(mod_anet_year))
...
```{r}
Fit_IAV_gpp_net = lmer(off ~ scale(gpp_net) + (1|id_site) + (1|species) , data = df_anl_pmodel, na.action = "na.exclude")
summary(Fit_IAV_gpp_net)
r.squaredGLMM(Fit_IAV_gpp_net)
plot(allEffects(Fit_IAV_gpp_net))

aic_iav_pm <- AIC(Fit_IAV_gpp_net)
out_plot <- allEffects(Fit_IAV_gpp_net)
gg_iav_pm <- ggplot_gpp_net(out_plot)
gg_iav_pm
```
...
```{r}
load("data/df_anl_pmodel.RData")
Fit_LT_gpp_net = lmer(off ~ scale(gpp_net) + scale(year) + (1|id_site) + (1|species), data = df_anl_pmodel, na.action = "na.exclude")
summary(Fit_LT_gpp_net)
r.squaredGLMM(Fit_LT_gpp_net)
plot(allEffects(Fit_LT_gpp_net))

aic_lt_pm <- AIC(Fit_LT_gpp_net)
out_plot_lt <- allEffects(Fit_LT_gpp_net)
gg_lt_pm_anet <- ggplot_gpp_net(out_plot_lt)
gg_lt_pm_year <- ggplot_year(out_plot_lt)

gg_lt_pm_anet + gg_lt_pm_year
```

Is this the better model than EOS ~ Anet?

```{r}
out_anova <- anova(Fit_IAV_gpp_net, Fit_LT_gpp_net)
out_anova
```
...
```{r}
mod_anet_year <- lmer(gpp_net ~ scale(year) + (1|id_site) + (1|species), data = df_anl, na.action = "na.exclude")
mod_anet_year <- lmer(gpp_net ~ year + (1|id_site) + (1|species), data = df_anl, na.action = "na.exclude")

out_plot <- allEffects(mod_anet_year)
gg_lt_anet_year <- ggplot_year0(out_plot)
...
gg_dual_pm <- ggplot_year_anet(x = out_plot_lt, y = out_plot_anet)
gg_dual_pm
```
...
pp <- (gg_iav_zz + gg_lt_zz_anet + gg_dual_zz)
