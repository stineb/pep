...
tab_model(fit_iav_pep_off_vs_on)
...
# Unscaled
trend_unscaled <- out$coefficients["scale(cA_tot)","Estimate"]/ sd(df_zz$cA_tot)
error_unscaled <- out$coefficients["scale(cA_tot)","Std. Error"]/ sd(df_zz$cA_tot)
upperCI_unscaled <- out$coefficients["scale(cA_tot)","Estimate"]/ sd(df_zz$cA_tot) + out$coefficient["scale(cA_tot)","Std. Error"]/ sd(df_zz$cA_tot)*1.96
lowerCI_unscaled <- out$coefficients["scale(cA_tot)","Estimate"]/ sd(df_zz$cA_tot) - out$coefficient["scale(cA_tot)","Std. Error"]/ sd(df_zz$cA_tot)*1.96
...
fit_iav_pep_off_vs_cAtotNS <- lmer(off ~ cA_tot + (1|id_site) + (1|species), data = df_zz, na.action = "na.exclude")
out_ns <- summary(fit_iav_pep_off_vs_cAtotNS)
print(out_ns)
upperCI <- out_ns$coefficients["cA_tot","Estimate"] + out_ns$coefficient["cA_tot","Std. Error"]*1.96
lowerCI <- out_ns$coefficients["cA_tot","Estimate"] - out_ns$coefficient["cA_tot","Std. Error"]*1.96

tab_model(fit_iav_pep_off_vs_cAtotNS)
```
...
tab_model(fit_lt_pep_off_vs_on_year)
...
# Bivariate model without scaling predictors
fit_lt_pep_off_vs_cAtot_yearNS = lmer(off ~ cA_tot + year + (1|id_site) + (1|species), data = df_zz, na.action = "na.exclude")
out_ns <- summary(fit_lt_pep_off_vs_cAtot_yearNS)
print(out_ns)
upperCI1 <- out_ns$coefficients["cA_tot","Estimate"] + out_ns$coefficient["cA_tot","Std. Error"]*1.96
lowerCI1 <- out_ns$coefficients["cA_tot","Estimate"] - out_ns$coefficient["cA_tot","Std. Error"]*1.96
upperCI2 <- out_ns$coefficients["year","Estimate"] + out_ns$coefficient["year","Std. Error"]*1.96
lowerCI2 <- out_ns$coefficients["year","Estimate"] - out_ns$coefficient["year","Std. Error"]*1.96
...
#### EOS ~ Anet LPJ + meanAnet LPJ

Instead of Year, look at the rolling year Anet. Expectation: positive relationship with mean-Anet, but negative with (interannual) Anet. Used for response to Constantin in Email.
...
df_zzz <- df_zz %>% 
  ...
  nest() %>% 
  ungroup() %>% 
  mutate(data = purrr::map(data, ~mutate(., cA_tot_mean = roll_mean(cA_tot, n = 10, align = "right", fill = NA)))) %>% 
  unnest(data)

fit_lt_pep_off_vs_cAtot_meanAtot = lmer(off ~ scale(cA_tot) + scale(cA_tot_mean) + (1|id_site) + (1|species), data = df_zzz, na.action = "na.exclude")
out <- summary(fit_lt_pep_off_vs_cAtot_meanAtot)
print(out)
# out$coefficients[,"Estimate"]
# out$coefficient[,"Std. Error"]*1.96
# upperCI1 <- out$coefficients["scale(cA_tot_anom)","Estimate"] + out$coefficient["scale(cA_tot_anom)","Std. Error"]*1.96
# lowerCI1 <- out$coefficients["scale(cA_tot_anom)","Estimate"] - out$coefficient["scale(cA_tot_anom)","Std. Error"]*1.96
# upperCI2 <- out$coefficients["scale(cA_tot_mean)","Estimate"] + out$coefficient["scale(cA_tot_mean)","Std. Error"]*1.96
# lowerCI2 <- out$coefficients["scale(cA_tot_mean)","Estimate"] - out$coefficient["scale(cA_tot_mean)","Std. Error"]*1.96
# r.squaredGLMM(fit_lt_pep_off_vs_cAtot_meanAtot)
# plot(allEffects(fit_lt_pep_off_vs_cAtot_meanAtot))
# tab_model(fit_lt_pep_off_vs_cAtot_meanAtot)
# 
# aic_lt_pep_off_vs_cAtot_meanAtot <- AIC(fit_lt_pep_off_vs_cAtot_meanAtot)
# out_lt_pep_off_vs_cAtot_meanAtot <- allEffects(fit_lt_pep_off_vs_cAtot_meanAtot)
# gg_lt_pep_off_vs_cAtot <- ggplot_cA_tot(out_lt_pep_off_vs_cAtot_meanAtot)
# gg_lt_pep_off_vs_year2 <- ggplot_year(out_lt_pep_off_vs_cAtot_meanAtot)
# 
# gg_lt_pep_off_vs_cAtot + gg_lt_pep_off_vs_year2
# 
# # Unscaled
# trend_unscaled <- out$coefficients["scale(cA_tot_mean)","Estimate"]/ sd(df_zz$year)
# error_unscaled <- out$coefficients["scale(cA_tot_mean)","Std. Error"]/ sd(df_zz$year)
# upperCI_unscaled <- out$coefficients["scale(cA_tot_mean)","Estimate"]/ sd(df_zz$year) + out$coefficient["scale(cA_tot_mean)","Std. Error"]/ sd(df_zz$year)*1.96
# lowerCI_unscaled <- out$coefficients["scale(cA_tot_mean)","Estimate"]/ sd(df_zz$year) - out$coefficient["scale(cA_tot_mean)","Std. Error"]/ sd(df_zz$year)*1.96
# 
# # Bivariate model without scaling predictors
# fit_lt_pep_off_vs_cAtot_meanAtotNS = lmer(off ~ cA_tot + cA_tot_mean + (1|id_site) + (1|species), data = df_zz, na.action = "na.exclude")
# out_ns <- summary(fit_lt_pep_off_vs_cAtot_meanAtotNS)
# print(out_ns)
# upperCI1 <- out_ns$coefficients["cA_tot","Estimate"] + out_ns$coefficient["cA_tot","Std. Error"]*1.96
# lowerCI1 <- out_ns$coefficients["cA_tot","Estimate"] - out_ns$coefficient["cA_tot","Std. Error"]*1.96
# upperCI2 <- out_ns$coefficients["cA_tot_mean","Estimate"] + out_ns$coefficient["cA_tot_mean","Std. Error"]*1.96
# lowerCI2 <- out_ns$coefficients["cA_tot_mean","Estimate"] - out_ns$coefficient["cA_tot_mean","Std. Error"]*1.96
...
Model NSC dynamics as a first-order decay process with turnover times $\tau = 1.5, 3, 5$. 
...
df_nsc <- df_zz %>% 
  group_by(sitename) %>%  # non-sensical column name
  nest() %>% 
  ungroup() %>% 
  mutate(data = purrr::map(data, ~mutate(., nsc_15 = calc_nsc(cA_tot, tau = 1.5))),
         data = purrr::map(data, ~mutate(., nsc_30 = calc_nsc(cA_tot, tau = 3.0))),
         data = purrr::map(data, ~mutate(., nsc_50 = calc_nsc(cA_tot, tau = 5.0)))) %>% 
  unnest(data)
```
...
```{r}
fit_lt_pep_off_vs_nsc_50_year = lmer(off ~ scale(nsc_50) + scale(year) + (1|id_site) + (1|species), data = df_nsc, na.action = "na.exclude")
out <- summary(fit_lt_pep_off_vs_nsc_50_year)
print(out)
out$coefficients[,"Estimate"]
out$coefficient[,"Std. Error"]*1.96
upperCI1 <- out$coefficients["scale(nsc_50)","Estimate"] + out$coefficient["scale(nsc_50)","Std. Error"]*1.96
lowerCI1 <- out$coefficients["scale(nsc_50)","Estimate"] - out$coefficient["scale(nsc_50)","Std. Error"]*1.96
upperCI2 <- out$coefficients["scale(year)","Estimate"] + out$coefficient["scale(year)","Std. Error"]*1.96
lowerCI2 <- out$coefficients["scale(year)","Estimate"] - out$coefficient["scale(year)","Std. Error"]*1.96
r.squaredGLMM(fit_lt_pep_off_vs_nsc_50_year)
plot(allEffects(fit_lt_pep_off_vs_nsc_50_year))
tab_model(fit_lt_pep_off_vs_nsc_50_year)

aic_lt_pep_off_vs_nsc_50_year <- AIC(fit_lt_pep_off_vs_nsc_50_year)
out_lt_pep_off_vs_nsc_50_year <- allEffects(fit_lt_pep_off_vs_nsc_50_year)
gg_lt_pep_off_vs_nsc_50 <- ggplot_nsc_50(out_lt_pep_off_vs_nsc_50_year)
gg_lt_pep_off_vs_year2 <- ggplot_year(out_lt_pep_off_vs_nsc_50_year)

gg_lt_pep_off_vs_nsc_50 + gg_lt_pep_off_vs_year2
...
# Bivariate model without scaling predictors
fit_lt_pep_off_vs_nsc_50_yearNS = lmer(off ~ nsc_50 + year + (1|id_site) + (1|species), data = df_zz, na.action = "na.exclude")
out_ns <- summary(fit_lt_pep_off_vs_nsc_50_yearNS)
print(out_ns)
upperCI1 <- out_ns$coefficients["nsc_50","Estimate"] + out_ns$coefficient["nsc_50","Std. Error"]*1.96
lowerCI1 <- out_ns$coefficients["nsc_50","Estimate"] - out_ns$coefficient["nsc_50","Std. Error"]*1.96
upperCI2 <- out_ns$coefficients["year","Estimate"] + out_ns$coefficient["year","Std. Error"]*1.96
lowerCI2 <- out_ns$coefficients["year","Estimate"] - out_ns$coefficient["year","Std. Error"]*1.96
```
...
tab_model(fit_iav_pep_off_vs_gppnet)
...
tab_model(fit_lt_pep_off_vs_gppnet_year)
...
tab_model(fit_iav_modis_off_vs_on)
...
tab_model(fit_iav_modis_off_vs_gppnet)
...
tab_model(fit_modis_anom_gppnet)
...
tab_model(fit_lt_modis_anom_on)
...
ff_lt_pep_off_vs_year2 <- gg_lt_pep_off_vs_year2 +
  labs(title = expression(paste("EOS ~ ", bold("Year"), " + ", italic("A")[net])), 
       ...
       #caption = expression(paste("2.525e"^"-1", " ± -2.173e"^"-3"))) +
...
ggsave("~/pep/manuscript/fig_1e.png", width = 8, height = 3, dpi=300)
```
...
ggsave("~/pep/manuscript/fig_2.png", width = 9, height = 8, dpi=300)
