...
map_gpp <- ggplot(data = world) + 
  geom_sf(fill= "grey") + #antiquewhite
  ...
  coord_sf(xlim = c(-180, 180), ylim = c(15, 75)) +
  geom_point(data = df_anl_modis_agg, aes(x = lon, y = lat, color = mean_gpp),size=.2) +
  labs(color=expression(paste(italic("A")[net]))) +
...
map_eos <- ggplot(data = world) + 
  ...
  annotation_north_arrow(location = "br", which_north = "true", 
                         ...
                         style = north_arrow_fancy_orienteering) + 
  coord_sf(xlim = c(-180, 180), ylim = c(15, 75)) +
  geom_point(data = df_anl_modis_agg, aes(x = lon, y = lat, color = mean_eos),size=.2) +
  labs(color="EOS") +
...
qq <- map_eos / map_gpp
ggarrange(pp,qq,ncol = 1, nrow = 2,align = "h",widths = c(0.1,3), heights = c(1,1.3))
ggsave("~/pep/manuscript/fig_2.png", width = 10, height = 10)
