...
pep725_pmodel <- readRDS("~/pep/data/pmodel_output/pep725_pmodel_output_21Jun.rds")
...
modis_pmodel <- readRDS("~/pep/data/pmodel_output/modis_pmodel_output_21Jun.rds")
#modis_pmodel_old <- readRDS("~/pep/data/pmodel_output/modis_pmodel_output_old.rds")
modis_pmodel <- modis_pmodel %>% 
  mutate(gpp_net = gpp - rd, 
         lue = gpp / apar)
# Join datasets
df_anl_modis <- modis_pheno_sites %>% 
  right_join(modis_pmodel[,2:10], by = c("lon","lat","year"))
df_anl_modis <- modis_pheno_sites %>% 
  inner_join(modis_pmodel[,2:10], by = c("lon","lat","year"))
...
#### Start-of-Season (SOS) - Appendix S1A
...
#### Long-term trend in Anet - Appendix S1C
...
```{r}
fit_iav_pep_off_vs_gppnet = lmer(off ~ scale(gpp_net) + (1|id_site) + (1|species), data = df_anl_pmodel_zz, na.action = "na.exclude")
fit_iav_pep_off_vs_gppnet = lmer(off ~ scale(gpp_net) + (1|id_site) + (1|sitename), data = df_anl_pmodel_zz, na.action = "na.exclude")
...
```{r}
fit_lt_pep_off_vs_gppnet_year = lmer(off ~ scale(gpp_net) + scale(year) + (1|id_site) + (1|species), data = df_anl_pmodel_zz, na.action = "na.exclude")
fit_lt_pep_off_vs_gppnet_year = lmer(off ~ scale(gpp_net) + scale(year) + (1|id_site) + (1|sitename), data = df_anl_pmodel_zz, na.action = "na.exclude")
...
#### Long-term trend in Anet - Appendix S1D

```{r}
fit_lt_pep_gppnet_vs_year <- lmer(gpp_net ~ scale(year) + (1|id_site) + (1|species), data = df_anl_pmodel_zz, na.action = "na.exclude")
...
df_anl_pmodel_zz <- df_anl_pmodel_zz %>% 
  group_by(sitename) %>% 
  nest() %>% 
  mutate(data = map(data, ~separate_anom(.))) %>% 
  unnest(data)

fit_lt_pep_anom_gppnet = lmer(off ~ scale(mean_gpp_net) + scale(anom_gpp_net) + scale(year) + (1|id_site) + (1|species), data = df_anl_pmodel_zz, na.action = "na.exclude")
...
#### Start-of-Season (SOS) - Appendix S2A
...
#### Long-term trend in Anet -  Appendix S2C
...
fit_modis_anom_gppnet = lmer(off ~ scale(mean_gpp_net) + scale(anom_gpp_net) + (1|sitename) + (1|year), data = df_anl_modis, na.action = "na.exclude")
...
fit_modis_anom_gppnet = lmer(off ~ scale(mean_gpp_net) + scale(anom_gpp_net) + (1|sitename) + (1|year), data = df_anl_modis, na.action = "na.exclude")  
  ...
  out_sum <- summary(fit_modis_anom_gppnet)
...
ggsave("~/pep/manuscript/fig_1.png", width = 9, height = 6)
...
```{r}
# Anomalies Year as random
gg_modis_mean_gppnet <- gg_modis_mean_gppnet +
  labs(title = expression(paste("EOS ~ ", bold("Mean "), bolditalic("A")[bold(net)], " + Anomalies ", italic("A")[net])), subtitle = "MODIS data and P-model") + ggtitle("A") 

gg_modis_anom_gppnet <- gg_modis_anom_gppnet +
  labs(title = expression(paste("EOS ~ Mean ", italic("A")[net], " + " ,bold("Anomalies "), bolditalic("A")[bold(net)])), subtitle = "MODIS data and P-model") + ggtitle("B") 
...
# Anomalies with Year as a fixed factor
...
plot_map3(df_anl_modis_agg, 
                varnam = "mean_gpp", 
                latmin = 15, latmax = 75,
                spacing = "constant", 
                #combine = FALSE, 
                colorscale = viridis::magma, 
                legend_title = expression(paste(italic("A")[net])),
                expand_size_y = 0.5)
...
ggplot(data = world) +
  ...
  coord_sf(xlim = c(-180, 180), ylim = c(15, 75))
...
map_gpp <- ggplot(data = world) + 
  ...
  ggtitle("D") + 
  geom_point(data = df_anl_modis_agg, aes(x = lon, y = lat, color = mean_gpp),size=.3) +
...
map_eos <- ggplot(data = world) + 
  ...
  coord_sf(xlim = c(-180, 180), ylim = c(15, 75), expand = F) +
  ggtitle("C") + 
  geom_point(data = df_anl_modis_agg, aes(x = lon, y = lat, color = mean_eos),size=.3) +
  labs(color="EOS (DOY)") +
  scale_color_viridis(option="viridis",limits=c(235,342), breaks= seq(260,340,20)) +
  theme(legend.position="right", #panel.grid.major = element_line(color = gray(.5), linetype = "dashed", size = 0.5), 
        panel.background = element_rect(fill = "aliceblue"),axis.title=element_blank(),plot.title = element_text(size = 10)) 

ggsave("~/pep/manuscript/map_eos.png", width = 8, height = 4)
ggsave("~/pep/manuscript/map_qpp.png", width = 8, height = 4)
ggsave("~/pep/manuscript/mean_anom.png", width = 6, height = 3)

# Plot together
pp <- gg_modis_mean_gppnet + gg_modis_anom_gppnet
qq <- map_eos / map_gpp

ggarrange(pp,qq,ncol = 1, nrow = 2,align = "v",widths = c(1/2,3), heights = c(1,1))
plot_grid(pp,qq,align = "v", ncol = 1, nrow = 2, rel_heights = c(1/2, 1),rel_widths = c(1/3, 1))
ggsave("~/pep/manuscript/fig_2.png", width = 7, height = 10)
```
