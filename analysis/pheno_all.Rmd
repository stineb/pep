...
modis_pheno_sites <- readRDS("~/pep/data/modis_pheno_sites.rds")
# Read p-model outputs
modis_pmodel <- readRDS("~/pep/data/modis_pmodel_output.rds")
...
df_anl_modis <- df_anl_modis %>% rename(on = SOS_doy, off = EOS_doy)
...
## gpp_net
Fit_LT_anom = lmer(off ~ scale(mean_gpp_net) + scale(anom_gpp_net) + scale(year) + (1|sitename), data = df_anl_modis, na.action = "na.exclude")
summary(Fit_LT_anom)
