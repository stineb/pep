...
save(df_anl, file = "data/df_anl.RData")
```
...
df_temporal %>% 
  ...
  xlim(-2.5, 2.5) +
  geom_vline(xintercept = 0, linetype = "dotted") +
  geom_vline(xintercept = median(df_temporal$coef_year_on, na.rm = TRUE), color = "red") +
...
df_temporal %>% 
  ...
  xlim(-2.5, 2.5) +
  geom_vline(xintercept = 0, linetype = "dotted") +
  geom_vline(xintercept = median(df_temporal$coef_year_off, na.rm = TRUE), color = "red") +
...
```{r}
load("data/df_temporal_12.RData")
df_temporal_12 %>% 
  # filter(p_value_on < 0.05) %>% 
  ggplot(aes(coef_on, ..density..)) +
  geom_histogram() +
